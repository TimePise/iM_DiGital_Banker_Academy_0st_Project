...
PAGE_PATHS = {
    ...
    "gnedar_heat":    "pages/01_세대와 성별에 따른 K-POP 히트곡의 지속력 차이.py",
    "meta":   "pages/02_K-POP 인기곡 메타 분석.py",
    "artist_population":   "pages/03_아티스트별 인기도 분석.py",
    "group_compare":   "pages/04_아이돌 그룹별 곡 특성 비교.py",
    "activity_population": "pages/05_K-POP 아티스트들의 활동량과 인기도 관계.py",
}
...
with row1[1]:
    st.page_link(PAGE_PATHS["gnedar_heat"], label="💖 히트곡 지속력 분석")
    st.caption("세대와 성별에 따른 히트곡 지속력 차이")

with row1[2]:
    st.page_link(PAGE_PATHS["meta"], label="📈 인기곡 메타분석")
    st.caption("인기곡 메타 분석")

row2 = st.columns(3)
with row2[0]:
    st.page_link(PAGE_PATHS["artist_population"], label="🎤 아티스트 인기도 분석")
    st.caption("아티스트별 인기도 분석")

with row2[1]:
    st.page_link(PAGE_PATHS["group_compare"], label="🎤 그룹별 곡 특성")
    st.caption("그룹별 곡 특성")

with row2[2]:
    st.page_link(PAGE_PATHS["activity_population"], label="✨ 활동량과 인기도")
    st.caption("활동량과 인기도 관계")
